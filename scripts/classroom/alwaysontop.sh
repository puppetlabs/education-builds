...
if [[ "$BASH_SOURCE" == "$0" ]]
then
    echo "This will keep your bash/zsh prompt at the top of the screen."
    echo "It's useful for presentations and demonstrations."
    echo
    echo "Other features:"
    echo "* clears the screen automatically with each command run"
    echo "* lists directory contents when entering them (abbreviated if needed)"
    echo "* shows the git or svn status in directories that have them"
    echo
    echo "Run the commands autotop and unautotop to enable or disable."
    echo
    echo -e "${COLOR_BRed}NOTICE:${COLOR_off} You should source this file instead of running it directly." > /dev/stderr
    echo
    exit 1
else
